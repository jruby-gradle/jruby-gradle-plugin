...
class GemVersionSpec extends Specification {
    void "parses single version"() {
        given:
        GemVersion subject = gemVersionFromGradleRequirement('1.2.3')
    ...
    void "parses single prerelease version"() {
        given:
        GemVersion subject = gemVersionFromGradleRequirement('1.2.pre')
    ...
    void "parses gradle semantic version second sample"() {
        given:
        GemVersion subject = gemVersionFromGradleRequirement('1.2.+')
    ...
    void "parses maven open version range"() {
        given:
        GemVersion subject = gemVersionFromGradleRequirement('[1.2.0,)')
    ...
    void "parses maven version range first sample"() {
        given:
        GemVersion subject = gemVersionFromGradleRequirement('(1.2.0.0, 1.2.4)')
    ...
    void "parses maven version range second sample"() {
        given:
        GemVersion subject = gemVersionFromGradleRequirement('(1.2.0, 1.2.4]')
    ...
    void "parses maven version range forth sample"() {
        given:
        GemVersion subject = gemVersionFromGradleRequirement('[1.2.1, 1.2.4]')
    ...
    void "parses maven version range trailing zeros"() {
        given:
        GemVersion subject = gemVersionFromGradleRequirement('[1.2.1.0.0.0, 1.2.4]')
    ...
    void "parses maven version range trailing zeros as prereleased version"() {
        given:
        GemVersion subject = gemVersionFromGradleRequirement('[1.2.1.0.pre.0, 1.2.4]')
    ...
    void "intersects two versions second sample reversed"() {
        given:
        GemVersion subject = gemVersionFromGradleRequirement('(1.2.1, 1.2.3)')
    ...
    void "intersects two versions with non lexical ordering"() {
        given:
        GemVersion subject = gemVersionFromGradleRequirement('[1.2.10, 1.2.14]')
    ...
    void "intersects two versions with different length"() {
        given:
        GemVersion subject = gemVersionFromGradleRequirement('[1.2, 1.2.14]')
    ...
    void "intersects two versions special one"() {
        given:
        GemVersion subject = gemVersionFromGradleRequirement('[0.9.0,0.9.99999]')
    ...
    void "intersects two versions with special full range"() {
        given:
        GemVersion subject = gemVersionFromGradleRequirement('[0,)')
    ...
    void "intersects two versions with workaround due to upstream bug"() {
        given:
        GemVersion subject = gemVersionFromGradleRequirement('(2.5.1.1,99999)')
    ...
    void "finds no conflicts in non-integer version ranges"() {
        given:
        GemVersion subject = gemVersionFromGradleRequirement('[1.2.bar, 1.2.foo]')
    ...
    void "does not throw an exception for a '+' version"() {
        when:
        gemVersionFromGradleRequirement('+').conflict()
